...
def train():
    ...
    print("Loading p208p2002/gpt2-squad-qg-hl")
    # Load the pre-trained openai-community/gpt2-large (teacher)
    teacher_model_name = "p208p2002/gpt2-squad-qg-hl"
    ...
    # Example: Load a dataset like "squad"
    ...
    def tokenize_function(examples):
        inputs = teacher_tokenizer(examples['context'] + "" + examples["question"], 
            return_tensors="pt", 
            padding="max_length", 
            truncation=True,
            max_length=512) 
        ...
        # Tokenize answers
        answers = [answer['text'][0] if len(answer['text']) > 0 else "" for answer in examples['answers']]
    ...
    student_model.save_pretrained(f"model-{model_name}_epochs-{num_epochs}_squad_alpha-{alpha}_beta-{beta}")
    student_tokenizer.save_pretrained(f"model-{model_name}_epochs-{num_epochs}_squad_alpha-{alpha}_beta-{beta}")
    
    plot_metrics(
        metrics={"loss": loss_history, "perplexity": ppl_history},
        run_tag="squad",
        out_dir=f"model-{model_name}_epochs-{num_epochs}_squad_alpha-{alpha}_beta-{beta}"
    )
